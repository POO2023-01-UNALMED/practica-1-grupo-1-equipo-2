...
from Calendario.gestionDatos import gestionDatos
...
if __name__ == "__main__":
    ...
    # Define los nombres de archivo de las imágenes
    image_files = [
        "imagen1.png",
        "imagen2.png",
        "imagen3.png",
    ...
    # Itera sobre los archivos de imagen y carga cada una
    for filename in image_files:
        # Obtén la ruta completa del archivo de imagen actual
        image_path = os.path.join(images_dir, filename)

        # Carga y procesa la imagen
        image = ImageTk.PhotoImage(Image.open(image_path).resize((150, 120)).convert("RGBA"))
    ...
    # Obtén la ruta completa del directorio de imágenes
    current_dir = os.path.dirname(os.path.abspath(__file__))
    images_dir = os.path.join(current_dir, "Images")
    ...
    # Crea las variables para almacenar las imágenes
    imagen_sis1 = None
    imagen_sis2 = None
    imagen_sis3 = None
    imagen_sis4 = None
    ...
    # Carga y procesa cada imagen
    for i, filename in enumerate(image_files):
        # Obtén la ruta completa del archivo de imagen actual
        image_path = os.path.join(images_dir, filename)

        # Carga y procesa la imagen
        image = ImageTk.PhotoImage(Image.open(image_path).resize((100, 120)).convert("RGBA"))

        # Asigna la imagen a la variable correspondiente
        if i == 0:
            imagen_sis1 = image
        elif i == 1:
            imagen_sis2 = image
